...
one. Just run the `/WILDSEQ/installers/install_miniforge3.sh` script and it will set up your environment. 
...
can run `/WILDSEQ/installers/install_wildseq_env.sh` to create a conda env for the script to use or manually 
install making sure you have all the package versions as specified in this script or as listed in
`/WILDSEQ/scripts/wildseq_dependencies.tsv`. 
...
A clean up script has been provided to quickly remove output files from the `/test_data/` directory. To run this, execute `/WILDSEQ/test_data/clean_test.sh`.
...
- **Sample and barcode information:**
  See `/WILDSEQ/test_data/test_data_analysis_sheet.csv`
- **Reference sequence GenBank accessions:**
  See `/WILDSEQ/scripts/ref_seqs/ref_seqs_source.csv` or in the **Methods section** of the paper.
- **qPCR primer sequences:**
  See **Methods section** of the paper.


## DEPENDENCIES
For dependencies see `/WILDSEQ/scripts/wildseq_dependencies.tsv`.
