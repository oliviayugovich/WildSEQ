...
# Calculate percent mapped reads (total reads) per barcode
barcode_totals <- species_calls %>%
  group_by(barcode) %>%
  summarise(total_barcode_reads = sum(reads), .groups = "drop")

# Join totals and calculate percent mapped for the top species
species_summary <- species_summary %>%
  left_join(barcode_totals, by = "barcode") %>%
  mutate(percent_mapped = (total_reads / total_barcode_reads) * 100)

# Define Inconclusive flag = <20 reads or <95% mapped reads
species_summary <- species_summary %>%
  mutate(inconclusive = total_reads < 20 | percent_mapped < 95)
...
species_summary <- species_summary %>%
                  ...
                  mutate(
                    species_fill = factor(ifelse(inconclusive, "Inconclusive", species_ID), # if Inconclusive, class as "Inconclusive", otherwise species_ID
      levels = c(
    ...
    species_label = ifelse(
      inconclusive, NA, species_ID), # dont add species label if Inconclusive
    reads_label = ifelse(
      inconclusive, "Inconclusive", as.character(total_reads)), # if Inconclusive labelled as Inconclusive
    reads_label_color = ifelse(
      inconclusive, "red", "black"), # format text colour of reads label: Inconclusive=red, otherwise black
    reads_label_face = ifelse(
      inconclusive, "bold", "plain"), # format text type of reads label: Inconclusive=bold, otherwise plain
    barcode = forcats::fct_rev(factor(barcode)), # reverse barcode levels so ascending order
    # CITES labelling
    `CITES Detected?` = case_when(
      inconclusive ~ "No", # if Inconclusive then NO
    ...
    `Species Group` = case_when(
      inconclusive ~ "Inconclusive", # if Inconclusive then Inconclusive 
    ...
    # Define Scientific Names
     `Scientific Name` = ifelse(inconclusive, "Inconclusive", species_ID))  # same as species_ID unless Inconclusive
...
top_percent <- df_bc$percent_mapped[which.max(df_bc$reads)]

top_species <- if (top_reads < 20 | top_percent < 95) {
  NA
} else {
  df_bc$species_ID[which.max(df_bc$reads)]
}
...
# Run summary table:
  ...
  # Make sure Sci Name correct for Inconclusives
species_summary <- species_summary %>% 
